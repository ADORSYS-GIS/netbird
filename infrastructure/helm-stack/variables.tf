...
variable "keycloak_admin_client_secret" {
  description = "Keycloak admin-cli client secret"
  type        = string
  sensitive   = true
  default     = ""
}
...
variable "create_db" {
  description = "Whether to create a new database instance (true) or use an existing one (false)"
  type        = bool
  default     = true
}

variable "external_db_dsn" {
  description = "Full PostgreSQL connection string (e.g., postgres://user:pass@ep-xyz.neon.tech/neondb?sslmode=require). Overrides Cloud SQL if provided."
  type        = string
  sensitive   = true
  default     = ""
}
