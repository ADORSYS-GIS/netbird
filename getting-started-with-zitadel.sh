...
# This line will tell the netbird to use -k only if it is for https

      if [[ "$NETBIRD_HTTP_PROTOCOL" == "https" ]]; then
        curl $FLAGS --fail --connect-timeout 1 -k -o /dev/null "$INSTANCE_URL/auth/v1/users/me" -H "Authorization: Bearer $PAT"
      else
...
# Check if we are running in a local/insecure environment
  if [ "$NETBIRD_DOMAIN" == "use-ip" ] || [ "$NETBIRD_DISABLE_LETSENCRYPT" == "true" ]; then
    ...
    # Original logic for public domain/secure setup
